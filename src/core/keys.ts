...
export function canonicalProjectId(workspaceRoot: string): string {
  ...
  normalized = normalized.replace(/^([a-z]):/i, (_m, d: string) => `${d.toLowerCase()}:`);
...
export function normalizeAgentName(name: string): string {
  ...
  if (!/^[a-z0-9][a-z0-9_-]{0,63}$/.test(trimmed)) {
    throw new AgentNameError(
      `Invalid agent name "${name}". Use 1-64 chars: lowercase letters, digits, '_' or '-'.`,
    );
  }
  return trimmed;
}
...
export function parseSessionKey(
  key: string,
): { version: string; projectId: string; taskNamespace?: string; agentName: string } | undefined {
  const parts = key.split(":");
  if (parts.length < 3 || parts[0] !== KEY_SCHEMA_VERSION) return undefined;
