...
function getServices(cwd) {
    ...
    const profiles = new ProfileStore({
        ...
        agentsDir: process.env.PI_SUBAGENTS_AGENTS_DIR ??
    ...
    const deliverer = {
        ...
        async deliver(caller, message, callId) {
            ...
            if (caller.kind === "root") {
                ...
                if (!target)
                    return false; // caller not live in this process: stays pending
        ...
        async confirm(callId, caller) {
            ...
            try {
                ...
                const found = raw.split("\n").some((line) => {
                    if (!line.includes(callId) || !line.includes(CALLBACK_TYPE))
                        return false;
                    try {
...
function makeDelegateTool() {
    return defineTool({
        ...
        parameters: Type.Object({
            ...
            prompt: Type.String({
                description: "The complete task for the agent. Task content only — do not forbid or " +
