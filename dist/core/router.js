...
export class DelegationRouter {
    ...
    async delegate(request, caller) {
        ...
        if (caller.sessionKey === targetKey) {
            ...
            return {
                ...
                targetAgent: profile.name,
                status: "queued",
                message: `Queued as a follow-up in your own session (${profile.name}). ` +
        ...
        if (caller.ancestry.includes(targetKey)) {
            const record = {
                ...
                createdAt: nowIso,
                settledAt: nowIso,
                summary: `Delegation to "${profile.name}" was rejected: it appears in the causal ` +
        ...
        return {
            ...
            targetAgent: profile.name,
            status: "queued",
            message: `Delegated to "${profile.name}" (call ${callId}). The result will arrive in ` +
    ...
    async flushOutbox() {
        ...
        for (const call of pending) {
            ...
            if (!ok)
                continue; // caller closed; retry on next flush
            await this.deps.calls.markDelivered(call.callId);
            delivered++;
        }
        return delivered;
    ...
    async runDelegatedTurn(session, record) {
    ...
    async tryDeliver(call, result) {
        try {
            const ok = await this.deps.deliverer.deliver(call.caller, renderCallback(call, result), call.callId);
            if (ok)
                await this.deps.calls.markDelivered(call.callId);
            return ok;
        }
    ...
    async runLocalFollowUp(session, record) {
        await this.deps.calls.update(record.callId, { status: "running" });
        try {
            ...
            await this.deps.calls.settle(record.callId, {
                status: "failed",
                summary: `local follow-up failed: ${err.message}`,
            });
        }
    }
}
export class UnknownAgentError extends Error {
    constructor(agent, known) {
...
export function autoModel(profile) {
...
export function renderCallback(call, result) {
    if (call.status === "needs_input") {
        const question = result?.question ?? call.summary ?? "(no question text)";
        return (`<delegation_question callId="${call.callId}" agent="${call.targetAgent}">\n` +
    ...
    const warning = result?.protocolWarning
        ...
        : "";
    const statusLine = call.status === "cycle_rejected"
    ...
    return (`<delegation_result callId="${call.callId}" agent="${call.targetAgent}" status="${call.status}">\n` +
        `${statusLine}\n\n${body}${warning}\n` +
        `</delegation_result>`);
}
/** Utility: is this session key part of this project? */
export function keyBelongsToProject(key, projectId) {
